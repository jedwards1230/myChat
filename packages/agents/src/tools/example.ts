...
async function list({ genre }: { genre: string }) {
	//logger.info("listing", { genre, functionName: "list" });
	return db
		.filter((item) => item.genre === genre)
		.map((item) => ({ name: item.name, id: item.id }));
}

export const runnableList: RunnableToolFunction<{ genre: string }> = {
	type: "function",
	function: {
		name: "list",
...
async function search({ name }: { name: string }) {
	//logger.info("searching for", { name, functionName: "search" });
	return db
		.filter((item) => item.name.includes(name))
		.map((item) => ({ name: item.name, id: item.id }));
}

export const runnableSearch: RunnableToolFunction<{ name: string }> = {
	type: "function",
	function: {
		name: "search",
		description:
